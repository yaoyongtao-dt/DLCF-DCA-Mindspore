...
from .util_api import float_tensor_2_bool_tensor, out_adaptor
...
def cat(tensors, dim=0, *, out=None):
    ...
    new_tensors = list(tensor for tensor in tensors if tensor.size != 0)
...
def flatten(input_tensor, start_dim=0, end_dim=-1):
    ...
    new_shape_list = []
    for i in shape_tuple[:start_dim]:
...
def bartlett_window(window_length, periodic=True, *, dtype=None, layout=None, device=None,
                    requires_grad=False):
    if periodic or layout is not None or device is not None or requires_grad is not False:
...
def histc(input, bins=100, min=0, max=0, *, out=None):
    ...
    result = mindspore.ops.HistogramFixedWidth(bins)(input, mindspore.Tensor([min, max], mindspore.float16))
    if input_min < min:
        less_than_min = ((input < min).astype(mindspore.float16)).sum()
        result[0] = result[0] - less_than_min
    if input_max < max:
        greater_than_max = ((input > max).astype(mindspore.float16)).sum()
...
def x2ms_sum(*args, **kwargs):
    def _sum(input, dim=None, keepdim=False, dtype=None):
        input_type = input.dtype
        if dim is None:
            dim = ()
        result = mindspore.ops.ReduceSum(keep_dims=keepdim)(input.astype(mindspore.float32), axis=dim)
        if dtype:
            result = result.astype(dtype)
...
def equal(input, other):
    ...
    return equal_ops(input, other).all()
...
def empty_like(input, *, dtype=None, layout=None, device=None, requires_grad=False, memory_format=None):
    if memory_format is not None or layout is not None or device is not None or requires_grad is not False:
...
def x2ms_all(input, dim, keepdim=False, *, out=None):
    result = input.all(axis=dim, keep_dims=keepdim)
    return out_adaptor(result, out)
...
def einsum(equation, *operands):
    np_arrays = list(input_tensor.asnumpy()
                     for input_tensor in operands)
    return mindspore.Tensor(np.einsum(equation, *np_arrays))
...
def topk(input, k, dim=None, largest=True, sorted=True, *, out=None):
    ...
    if dim not in [-1, input.dim() - 1, None]:
        ...
        values, indices = mindspore.ops.TopK(sorted=sorted)(input, k)
    ...
    if not largest:
        values, indices = mindspore.ops.TopK(sorted=sorted)(input, k)
        return values * -1, indices
    return mindspore.ops.TopK(sorted=sorted)(input, k)
...
def nonzero(input, out=None, as_tuple=False):
    result = np.nonzero(input.asnumpy())
    if as_tuple:
        result = tuple(mindspore.Tensor(index) for index in result)
    elif len(result[0]) == 0:
        result = mindspore.ops.Zeros()((0, len(result)), mindspore.int32)
    else:
        result = mindspore.Tensor(np.array(result).T)
    return out_adaptor(result, out)
...
def tril(input, diagonal=0, *, out=None):
    result = mindspore.nn.Tril()(input, diagonal)
    return out_adaptor(result, out)
...
def take(input, index):
    ...
    result = mindspore.Tensor.take(mindspore.Tensor.flatten(input), index)
    return result
