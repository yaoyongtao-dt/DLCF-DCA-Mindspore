...
from .loss import CrossEntropyLoss, SmoothL1Loss, legacy_parameter
...
def tensor_max_pool2d(tensor, kernel_size, stride=None, padding=0, dilation=1, return_indices=False, ceil_mode=False):
    ...
    if kernel_size == 2 * padding + 1 or (ceil_mode and padding == 0):
        max_pool2d_func = mindspore.nn.MaxPool2d(kernel_size=kernel_size, stride=stride, pad_mode="same")
        return max_pool2d_func(tensor)
    elif padding == 0:
        max_pool2d_func = mindspore.nn.MaxPool2d(kernel_size=kernel_size, stride=stride, pad_mode="valid")
        return max_pool2d_func(tensor)
    else:
