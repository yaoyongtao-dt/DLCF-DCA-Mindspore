...
from mindformers import BertModel, AutoTokenizer
...
class TextClassifier:
            ...
            try:
                ...
                if state_dict_path:
                    if 'pretrained_bert_name' in self.opt.args or 'pretrained_bert' in self.opt.args:
                        ...
                        self.model = x2ms_adapter.to(self.opt.model(self.embedding_matrix, self.opt), self.opt.device)
    ...
    def batch_infer(self,
        ...
        self.infer_dataloader = x2ms_datasets.DataLoader(dataset=self.dataset, batch_size=1, shuffle=False)
        ...
        for _, sample in enumerate(self.infer_dataloader):
            ...
            for i, i_probs in enumerate(t_probs):
                if 'origin_label_map' in self.opt.args:
                    label = self.opt.origin_label_map[int(x2ms_adapter.tensor_api.argmax(i_probs, axis=-1))]
                    real_label = self.opt.origin_label_map[int(sample['label'][i])]
                else:
                    label = int(x2ms_adapter.tensor_api.argmax(i_probs, axis=-1))
