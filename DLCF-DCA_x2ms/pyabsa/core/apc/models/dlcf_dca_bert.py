...
def weight_distrubute_local(bert_local_out, depend_weight, depended_weight, depend_vec, depended_vec, opt):
    ...
    bert_local_out2 = bert_local_out2 + x2ms_adapter.mul(bert_local_out, depend_vec2) + x2ms_adapter.mul(bert_local_out,
                                                                                                         depended_vec2)
...
class DLCF_DCA_BERT(nn.Cell):
    ...
    def weight_calculate(self, sa, pool, lin, d_w, ded_w, depend_out, depended_out):
        ...
        for i in range(x2ms_adapter.tensor_api.x2ms_size(depend_weight)[0]):
            ...
            depended_weight[i] = x2ms_adapter.tensor_api.item(depended_weight[i]) * x2ms_adapter.tensor_api.item(
                ded_w[i])
            weight_sum = x2ms_adapter.tensor_api.item(depend_weight[i]) + x2ms_adapter.tensor_api.item(
                depended_weight[i])
            if weight_sum != 0:
    ...
    def construct(self, inputs):
        ...
        # input_mask = lcf_matrix
        # token_type_id = lcf_matrix
        # print(inputs[0])
        # 报错
