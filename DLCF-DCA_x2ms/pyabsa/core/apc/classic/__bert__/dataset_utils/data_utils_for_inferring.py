    ...
    def process_data(self, samples, ignore_error=True):
        ...
        for text in tqdm.tqdm(samples, postfix='building word indices...'):
            try:
                ...
                left_len = min(self.opt.max_seq_len - aspect_len, x2ms_adapter.tensor_api.x2ms_sum(np, left_indices != 0))
                left_indices = x2ms_np.concatenate((left_indices[:left_len], np.asarray([0] * (self.opt.max_seq_len - left_len))))
