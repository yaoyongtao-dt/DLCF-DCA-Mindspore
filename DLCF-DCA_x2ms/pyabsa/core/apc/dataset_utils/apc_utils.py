...
def prepare_input_for_apc(opt, tokenizer, text_left, text_right, aspect):
    if hasattr(opt, 'dynamic_truncate') and opt.dynamic_truncate:
        ...
        # test code
        text_left = ' '.join(x2ms_adapter.tensor_api.split(text_left, ' ')[int(-(opt.max_seq_len - len(x2ms_adapter.tensor_api.split(aspect))) / 2) - 1:])
        text_right = ' '.join(x2ms_adapter.tensor_api.split(text_right, ' ')[:int((opt.max_seq_len - len(x2ms_adapter.tensor_api.split(aspect))) / 2) + 1])
    ...
    # print("测试")
    # tokenizer.cls_token = tokenizer.cls_token if tokenizer.cls_token else '[CLS]'
    # tokenizer.f = tokenizer.sep_token if tokenizer.sep_token else '[SEP]'
    # bos_token = tokenizer.cls_token
    ...
    text_raw = text_left + ' ' + aspect + ' ' + text_right
    # 处理这里的奇怪之处
    text_spc = text_raw + ' ' + aspect

    # 测试----------------------------------------
    # tests = tokenizer(text_spc, max_length=opt.max_seq_len, padding="max_length")
    # attention_mask = tests['attention_mask']
    # token_type_ids = tests['token_type_ids']
    attention_mask = np.zeros(opt.max_seq_len)
    token_type_ids = np.zeros(opt.max_seq_len)
    # text_bert_indices = tests['input_ids']
    text_bert_indices = text_to_sequence(tokenizer, text_spc, opt.max_seq_len)
    # text_bert_indices =tokenizer(text_spc, opt.max_seq_len)
    ...
    ################################################################################yyt修改
    # text_raw = text_left + ' ' + aspect + ' ' + text_right
    # text_spc = cls_token + ' ' + text_raw + ' ' + sep_token + ' ' + aspect + ' ' + sep_token
    # text_bert_indices = text_to_sequence(tokenizer, text_spc, opt.max_seq_len)
    # text_raw_bert_indices = text_to_sequence(tokenizer, cls_token + ' ' + text_raw + ' ' + sep_token, opt.max_seq_len)
    # aspect_bert_indices = text_to_sequence(tokenizer, aspect, opt.max_seq_len)
    #
    # aspect_begin = len(tokenizer.tokenize(cls_token + ' ' + text_left))
    # aspect_position = set(range(aspect_begin, aspect_begin + np.count_nonzero(aspect_bert_indices)))
    ################################################################################yyt修改
...
def calculate_dep_dist(sentence, aspect):
    terms = [a.lower() for a in x2ms_adapter.tensor_api.split(aspect)]
    try:
        doc = nlp(sentence)
