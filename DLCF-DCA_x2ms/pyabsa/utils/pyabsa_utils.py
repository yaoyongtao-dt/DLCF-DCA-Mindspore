    ...
    if from_checkpoint_path:
        ...
        if from_checkpoint_path:
            ...
            if model_path:
                ...
                trainer.model = x2ms_adapter.load(model_path[0])
            if state_dict_path:
                x2ms_adapter.load_state_dict(trainer.model, x2ms_adapter.load(state_dict_path[0]))
                trainer.model.opt = trainer.opt
                x2ms_adapter.to(trainer.model, trainer.opt.device)
            else:
...
def validate_version():
    ...
    if response.status_code == 200:
        ...
        x2ms_adapter.tensor_api.sort(versions, key=parse_version, reverse=True)
