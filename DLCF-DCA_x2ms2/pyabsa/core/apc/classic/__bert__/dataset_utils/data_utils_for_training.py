...
def _load_word_vec(path, word2idx=None, embed_dim=300):
    ...
    for line in tqdm.tqdm(fin, postfix='Loading embedding file...'):
        tokens = x2ms_adapter.tensor_api.split(line.rstrip())
...
class BERTBaselineABSADataset:
    ...
    def __init__(self, dataset_list, tokenizer, opt):
        ...
        for i in tqdm.tqdm(range(0, len(lines), 3), postfix='building word indices...'):
            ...
            aspect_len = x2ms_adapter.tensor_api.x2ms_sum(np, aspect_indices != 0)
            left_len = min(opt.max_seq_len - aspect_len, x2ms_adapter.tensor_api.x2ms_sum(np, left_indices != 0))
            left_indices = x2ms_np.concatenate((left_indices[:left_len], np.asarray([0] * (opt.max_seq_len - left_len))))
