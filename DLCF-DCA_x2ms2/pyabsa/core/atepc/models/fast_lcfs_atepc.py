...
class FAST_LCFS_ATEPC(BertForTokenClassification):
    ...
    def __init__(self, bert_base_model, opt):
        super(FAST_LCFS_ATEPC, self).__init__(config=bert_base_model.config)
    ...
    def construct(self, input_ids_spc,
        ...
        if lcf_cdm_vec is not None or lcf_cdw_vec is not None:
            ...
            if 'cdm' in self.opt.lcf:
                cdm_context_out = x2ms_adapter.mul(global_context_out, lcf_cdm_vec)
                ...
                cdw_context_out = x2ms_adapter.mul(global_context_out, lcf_cdw_vec)
